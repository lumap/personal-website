...
async function handleReq(req: Request, server: Server) {
    ...
    switch (match[1] || "www") {
        ...
        case "c": {
            return handleCDN(req, pathAccessed, match[2], server);
        }
        case "a": {
            return await handleAPI(req, pathAccessed, match[2], server);
        }
        case "www": {
            return await handleWWW(req, pathAccessed, match[2], server);
        }
        default: {
            return new Response();
        }
